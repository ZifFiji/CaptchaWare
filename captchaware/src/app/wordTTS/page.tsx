...
export default function WordTTS() {
  ...
  return (
    <div className="h-screen w-full p-4">
      <p className="mb-4 text-center">You need to write every word without any spelling mistakes. Every word must start with an uppercase letter.</p>
      {gameComplete ? (
        ...
        <div className="flex flex-col items-center justify-center h-64">
          <button type="button" onClick={() => setIsStart(true)} className="bg-green-500 text-white px-6 py-3 rounded text-lg hover:bg-green-600">
            Are you ready?
          </button>
        </div>
      ) : (
        <div className="h-full w-full">
          <div className="text-center mb-4">
            <p className="mb-2">
              Word {indexWord + 1} of {listOfHardestWord.length}
            </p>
            <p className="mb-2">Number of fails: {countFails}</p>
            <button type="button" onClick={playSound} className="bg-blue-500 text-white px-4 py-2 rounded mr-4 hover:bg-blue-600">
              🔊 Play word
            </button>
          </div>
          <div className="text-center mb-4">
